...
extern LPDIRECTINPUTDEVICE8			G_Mouse;			// The mouse device
extern DIMOUSESTATE					G_MouseState;		// The mouse state
extern LPD3DXSPRITE					G_SpriteHandler;	// spriteHandler to draw texture
extern LPDIRECTSOUND8				G_SoundManager;
...
struct GPoint2D
{
	...
	GPoint2D();
	GPoint2D(float, float);
	void GPoint2D::operator=(const GPoint2D & point);
	friend bool operator==(const GPoint2D & lhs, const GPoint2D & rhs);
};
...
struct GRect
{
	...
	GRect();
	GRect(float a_x, float a_y, int a_width, int a_heigh, float vx, float vy);
