...
SceneLevel1Scene3::SceneLevel1Scene3() {}

void SceneLevel1Scene3::RenderFrame()
{
...
void SceneLevel1Scene3::ProcessInput()
{
...
void SceneLevel1Scene3::LoadResources()
{
	_bg = new GTiledBG();
	_bg->GetMapFromFile("SceneLevel1Scene2_2BG.txt");
	_bg->LoadImageFromFile("statemapTS.png");
	CObjectManager::GetInstance()->GetObjectsFromFile("SceneLevel1Scene2_2GO.txt");
	...
	_camera = new GCamera();
	_camera->SetSizeMap(1060, G_ScreenWidth);
	_camera->viewport.x = 0;
	_camera->viewport.y = G_ScreenHeight;
	//_quadTree = new GQuadtree();
}

void SceneLevel1Scene3::Update(int a_deltaTime)
{
	//_quadTree->Clear();
