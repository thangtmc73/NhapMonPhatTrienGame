...
SceneLevel1Scene2::SceneLevel1Scene2() {}

void SceneLevel1Scene2::RenderFrame()
{
...
void SceneLevel1Scene2::ProcessInput()
{
...
void SceneLevel1Scene2::LoadResources()
{
	_bg = new GTiledBG();
	_bg->GetMapFromFile("SceneLevel1Scene2_1BG.txt");
	_bg->LoadImageFromFile("statemapTS.png");
	CObjectManager::GetInstance()->GetObjectsFromFile("SceneLevel1Scene2_1GO.txt");
	_bg->SetArrayDimensions(121, 10);
	...
	_camera = new GCamera();
	_camera->SetSizeMap(3904,G_ScreenWidth);
	_camera->viewport.x = 0;
	_camera->viewport.y = G_ScreenHeight;
	//_quadTree = new GQuadtree();
}

void SceneLevel1Scene2::Update(int a_deltaTime)
{
	//_quadTree->Clear();
