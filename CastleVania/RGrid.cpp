#include "RGrid.h"
#include "RBaseObject.h"
#include "Utils.h"

RGrid::RGrid()
{
	_grid = NULL;
}

RGrid::~RGrid()
{
	if (_grid != NULL)
	{
		for (int i = 0; i < _gridCols; i++)
		{
			_grid[i].clear();
		}
		delete[] _gridRect;
		delete[] _grid;
		delete[] _backgroundObject;
		_grid = NULL;
	}
}

void RGrid::setInitialInfo(int a_mapWidth, int a_mapHeight, int a_cellWidth, int a_cellHeight)
{
	if (_grid == NULL)
	{
		_cellWidth = a_cellWidth;
		int rowDiv = a_mapHeight % a_cellHeight;
		int colDiv = a_mapWidth % a_cellWidth;
		_gridCols = colDiv == 0 ? (int)(a_mapWidth / a_cellWidth) : (int)((a_mapWidth / a_cellWidth) + 1);
		_grid = new std::vector<RObject*>[_gridCols];
		_backgroundObject = new std::vector<RObject*>[_gridCols];
		_gridRect = new GRect[_gridCols];
		for (int i = 0; i < _gridCols; i++)
		{
			_gridRect[i].width = a_cellWidth;
			_gridRect[i].height = a_cellHeight;
			_gridRect[i].position.x = i * a_cellWidth;
			_gridRect[i].position.y = a_cellHeight;
		}
	}
}

void RGrid::clearData()
{
	for (int i = 0; i < _gridCols; i++)
	{
		_grid[i].clear();
	}
}

void RGrid::clear()
{
	if (_grid != NULL)
	{
		for (int i = 0; i < _gridCols; i++)
		{
			_grid[i].clear();
		}
		delete[] _gridRect;
		delete[] _grid;
		_grid = NULL;
	}
}

std::set<RObject*> RGrid::getListGameObjectUpdate(GRect rect)
{
	int indexTopLeftCol = (int)(rect.position.x / _cellWidth);
	int indexRightBottomCol = (int)((rect.position.x + rect.width) / _cellWidth);
	if (indexRightBottomCol == _gridCols)
	{
		indexRightBottomCol = _gridCols - 1;
	}
	std::set<RObject*> listObject;
	for (int i = indexTopLeftCol; i <= indexRightBottomCol; i++)
	{
		for (int j = 0; j < _grid[i].size(); j++)
		{
			if (std::find(listObject.begin(), listObject.end(), _grid[i].at(j)) == listObject.end())
			{
				listObject.insert(_grid[i].at(j));
			}
		}
		for (int j = 0; j < _backgroundObject[i].size(); j++)
		{
			if (std::find(listObject.begin(), listObject.end(), _backgroundObject[i].at(j)) == listObject.end())
			{
				listObject.insert(_backgroundObject[i].at(j));
			}
		}
	}
	return listObject;
}

void RGrid::retrieve(RObject * obj)
{
	GRect boundingBox = obj->getTopLeftBoundingBox();
	int indexTopLeftCol = (int)(boundingBox.position.x / _cellWidth);
	int indexRightBottomCol = (int)((boundingBox.position.x + boundingBox.width) / _cellWidth);
	if (indexRightBottomCol == _gridCols)
	{
		indexRightBottomCol = _gridCols - 1;
	}
	for (int i = indexTopLeftCol; i <= indexRightBottomCol; i++)
	{
		if (obj->isBackgroundObject()) {
			_backgroundObject[i].push_back(obj);
		}
		else {
			_grid[i].push_back(obj);
		}
	}
}

void RGrid::clearNonBackgroundObject()
{
	for (int i = 0; i < _gridCols; i++)
	{
		_grid[i].clear();
	}
}
